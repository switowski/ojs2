...
<tbody>
	...
	{/if}
		<tr valign="top">
			...
			{if $isEditor}
				<td><a onclick="return confirm('Do you really want to remove it?')" href="{url page="editor" op="deleteEditAssignment" path=$editAssignment->getEditId()}" class="actionbtn btn btn-xs btn-default" style="min-width:70px"><i class="material-icons" style="float:left;font-size:16px;padding-right:5px;">delete</i> {translate key="common.delete"}</a></td>
			{/if}
		</tr>
	{foreachelse}
