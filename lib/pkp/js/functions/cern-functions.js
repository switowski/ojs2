...
function defaultStyle() {	
	...
	$('img[alt="Mail"]').each(function() {
		if($(this).parent().is("a")) {
			...
			$(newButton).addClass("btn btn-default btn-xs disabled").html('<i class="material-icons" style="font-size: 18px;" title="Email">email</i>');
			$(this).parent().html(newButton);
		}
	});
	
	//Change all input[file]
	//Apply style bootstrap ignoring the ones that have the class ignore-file 
	$(":file:not(.ignore-file)").filestyle({buttonBefore: true, iconName: "file_upload"});
	...
	//Fixing layout of ignore-file, because bootstrap filestyle does not work on them 
	$(".ignore-file ").parent().html(
			'<div class="col-md-10"><input type="file" class="uploadField" name="submissionFile" id="submissionFile"></div> <div class="col-md-2"> <input name="uploadSubmissionFile" type="submit" class="button btn btn-default btn-sm" value="Upload"></div>');
}
...
/**
 * This function float the elements to left that have the class to-left when
 * the size of the window is less than 990
 */

function floatLeftOnMobile() {
	if($( document ).width() < 990) {
		$(".to-left").css("float","left");
	}
}
