...
function defaultStyle() {	
	...
	$('img[alt="Mail"]').each(function() {
		if($(this).parent().is("a")) {
			...
			$(this).parent().html('<i class="material-icons" style="font-size: 15px;" title="Email">email</i>');
		} else {
			var newButton = document.createElement('button');
			$(newButton).addClass("btn btn-default btn-xs disabled").html('<i class="material-icons" style="font-size: 15px;" title="Email">email</i>');
	...
	$(":file").each(function() {
		...
		$(this).parent().find(".bootstrap-filestyle").find(".btn-default").removeClass("btn-default").addClass("btn-primary");
		
		if($(button).is(":submit")) { //It has an upload button
			$(this).parent().find(":text").css("width","70%");
...
function submissionCheckbox() {
	...
	if(typeof checkbox != 'undefined') {
		...
		$(parent).next().addClass("col-xs-11");
		if($( document ).width() >= 990) {
			$(parent).next().css("margin-left","-50px");
		}
